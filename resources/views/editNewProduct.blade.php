...
<html lang="en">
  ...
  <body>
    <div class="container">
      ...
      <h1 class="title">Edit Newest Product</h1>


      <form action="/update-newest-product/{{ $NewProduct->id }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('patch')
        <div class="foto-box">
        ...
        <div class="mb-3">
          ...
          <input type="file" class="form-control foto-input" id="FotoNew" aria-describedby="emailHelp" value="{{ $NewProduct->FotoNew }}" name="FotoNew">
          
        <table>
          <tr>
            <td><label for="HargaNew" class="form-label">Harga</label></td>
            <td><input type="text" class="form-control w-100"   id="HargaNew" aria-describedby="emailHelp" value="{{ $NewProduct->HargaNew }}" name="HargaNew">
            @error('HargaNew')
              <p style="color: red">{{ $message }}</p>
            @enderror</td>
          </tr>
          <tr>
            ...
            <td><input type="text" class="form-control w-100" id="NamaNew" aria-describedby="emailHelp" value="{{ $NewProduct->NamaNew }}" name="NamaNew">
            @error('NamaNew')
              <p style="color: red">{{ $message }}</p>
            @enderror</td>
          </tr>
          <tr>
            ...
            <td><input type="text" class="form-control w-100" id="DescriptionNew" aria-describedby="emailHelp" value="{{ $NewProduct->DescriptionNew }}" name="DescriptionNew">
